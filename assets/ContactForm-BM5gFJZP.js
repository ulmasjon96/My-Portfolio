const __vite__mapDeps = (
	i,
	m = __vite__mapDeps,
	d = m.f || (m.f = ['assets/index-Cq85Tfvu.js', 'assets/radix-1h7f9VDF.js', 'assets/style-DpsRipQV.css']),
) => i.map(i => d[i]);
import { _ as h, b as d } from './index-DgPIJX3V.js';
import { j as n, R as T, r as l } from './radix-1h7f9VDF.js';
import { S as E } from './section-title-Cm89KhRi.js';
import { u as D } from './i18n-Blv0dhYd.js';
import './motion-eszmwozl.js';
const _ = '/My-Portfolio/assets/formBg-D9m73Ati.jpg';
function I({ label: e, name: r, type: t = 'text', placeholder: i }) {
	const a = `${r}-label`,
		o = `${r}-hint`;
	return n.jsxs('div', {
		className: 'relative w-full group my-10',
		children: [
			n.jsx('input', {
				id: r,
				name: r,
						...
						: r === 'email'
							...
							: r === 'username'
								...
								: r === 'company'
									? 'organization'
									: void 0,
				inputMode: t === 'email' ? 'email' : 'text',
				enterKeyHint: t === 'email' ? 'send' : 'next',
				spellCheck: t === 'email' ? 'false' : 'true',
				placeholder: i,
				className: `\r
        ...
        `,
			...
			n.jsxs('span', { id: o, className: 'sr-only', children: [e, ' field'] }),
			n.jsx('span', {
				className: `\r
...
\r
      ...
      `,
			...
			n.jsx('div', {
				className: 'absolute bottom-2 left-5 right-14 h-1 flex gap-[2px] opacity-0 peer-focus:opacity-100',
				children: Array.from({ length: 18 }).map((s, c) =>
					n.jsx(
						'span',
						{ className: 'flex-1 bg-cyan-400/40 animate-equalizer', style: { animationDelay: `${c * 0.12}s` } },
						c,
					),
				),
			}),
...
w.displayName = 'PhoneField';
const O = l.lazy(() => h(() => import('./index-Cq85Tfvu.js'), __vite__mapDeps([0, 1])));
function A({ value: e, onChange: r, label: t, placeholder: i }) {
	const [a, o] = l.useState(!1);
	return (
		l.useEffect(() => {
			(h(() => Promise.resolve({}), __vite__mapDeps([2])), o(!0));
		}, []),
		n.jsxs('div', {
			className: `\r
...
`,
			children: [
				n.jsx(l.Suspense, {
					fallback: n.jsx('div', {
						className: 'h-[50px] w-full rounded-md bg-[#00162ed0] animate-pulse border border-cyan-400/20',
					}),
					children:
						a &&
						n.jsx(O, {
      ...
      `,
				...
				n.jsx('div', {
					className: 'absolute bottom-2 left-5 right-14 h-1 flex gap-[2px] opacity-0 group-focus-within:opacity-100',
					children: Array.from({ length: 18 }).map((s, c) =>
						n.jsx(
							'span',
							{ className: 'flex-1 bg-cyan-400/40 animate-equalizer', style: { animationDelay: `${c * 0.12}s` } },
...
function R({ label: e, name: r, id: t, placeholder: i }) {
	const a = t || r;
	return n.jsxs('div', {
		className: 'relative w-full group my-12',
		children: [
			n.jsx('textarea', {
				id: a,
				name: r,
...
`,
      ...
      `,
			...
			n.jsx('div', {
				className: 'absolute bottom-3 left-5 right-14 h-1 flex gap-[2px] opacity-0 peer-focus:opacity-100',
				children: Array.from({ length: 20 }).map((o, s) =>
					n.jsx(
						'span',
						{ className: 'flex-1 bg-cyan-400/40 animate-equalizer', style: { animationDelay: `${s * 0.1}s` } },
						s,
					),
				),
			}),
...
const $ = '8423383975:AAHPPQ8K7BCiWkaVtvqIO9J53q0Emiqhwo4',
	q = '560000221';
function b(e) {
	return e
		? (typeof e == 'string' ? e : e.name).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
		: '';
}
function K() {
	const { t: e } = D(),
	...
	l.useEffect(() => {
		const p = new Image();
		((p.src = _), (p.onload = () => P(!0)));
	}, []);
	const v = async p => {
		if ((p.preventDefault(), r)) return;
		const u = p.currentTarget;
		if (!u.checkValidity()) {
			(d.error(e('form.errorValidation'), { description: e('form.errorValidationDesc') }), u.reportValidity());
			return;
		}
		const { isValidPhoneNumber: C } = await h(
			async () => {
				const { isValidPhoneNumber: m } = await import('./index-Cq85Tfvu.js');
				return { isValidPhoneNumber: m };
			},
			__vite__mapDeps([0, 1]),
		);
		if (!o || !C(o)) {
			d.error(e('form.errorPhone'), { description: e('form.errorPhoneDesc') });
			return;
		}
		const j = u.querySelector('input[name="email"]')?.value.trim() || '';
		if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(j)) {
			d.error(e('form.errorEmail'), { description: e('form.errorEmailDesc') });
			return;
		}
		...
		try {
			const y = await (
				await fetch(`https://api.telegram.org/bot${$}/sendMessage`, {
					...
					body: JSON.stringify({ chat_id: q, text: k, parse_mode: 'HTML', disable_web_page_preview: !0 }),
				})
			).json();
			if (!y.ok) throw new Error(y.description);
			(d.success(e('form.success'), { description: e('form.successDesc') }),
				a(!0),
				u.reset(),
				s(''),
				setTimeout(() => a(!1), 3e3));
		} catch (m) {
			(console.error(m), d.error(e('form.errorSend'), { description: e('form.errorSendDesc') }));
		} finally {
			t(!1);
		}
	};
	return n.jsxs('section', {
		...
		children: [
			n.jsx('div', {
				...
				style: c ? { backgroundImage: `url(${_})` } : void 0,
			}),
			n.jsx('div', {
				className: 'absolute inset-0 bg-gradient-to-b from-background/30 via-background/5 to-background',
			}),
			n.jsx('div', {
				...
				children: n.jsxs('div', {
					...
					children: [
						n.jsx(E, { number: e('contact.number'), title: e('contact.title') }),
						n.jsxs('div', {
							className: 'w-full max-w-[530px] relative z-10 md:ml-auto',
							children: [
								n.jsx('style', {
									children: `
              @keyframes scan {
            ...
            `,
								...
								n.jsx('h1', {
									className:
										'text-[40px] xs:text-[50px] sm:text-[65px] md:text-[65px] lg:text-[70px] leading-tight tracking-[2px] sm:tracking-[3px] md:tracking-[5px] text-center font-brush break-words pb-2 sm:mb-6 md:mb-8 bg-gradient-to-r from-transparent via-cyber-3 to-transparent bg-[length:90%] bg-no-repeat text-transparent bg-clip-text animate-textGlow',
            ...
            `,
									children: [
										...
										n.jsx(A, {
											...
											value: o,
											onChange: s,
											placeholder: e('form.phonePlaceholder'),
										}),
										n.jsx(I, {
											label: e('form.email'),
											name: 'email',
										...
										n.jsx(R, { label: e('form.message'), name: 'message', placeholder: e('form.messagePlaceholder') }),
										n.jsxs('div', {
											...
											children: [
												n.jsx('button', {
													...
													disabled: r,
													className:
														'w-full py-3 px-3 text-sm sm:text-base bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))] font-bold uppercase tracking-[2px] transition-all duration-300 shadow-[var(--shadow-glow)] hover:scale-105 active:scale-95 disabled:opacity-50',
													children: e(r ? 'form.sending' : i ? 'form.sent' : 'form.submit'),
												}),
												n.jsx('button', {
													type: 'reset',
													onClick: () => s(''),
													className:
														'w-full py-3 px-3 text-sm sm:text-base bg-[hsl(var(--secondary))] text-[hsl(var(--foreground))] border border-[hsl(var(--border))] font-bold uppercase tracking-[2px] transition-all duration-300 hover:bg-[hsl(var(--accent)/0.2)]',
													children: e('form.reset'),
												}),
											],
										}),
									],
