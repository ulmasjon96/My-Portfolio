import { r as k, a as Te } from './radix-1h7f9VDF.js';
const g = r => typeof r == 'string',
	M = () => {
		let r, e;
	...
	Ve = /###/g,
	he = r => (r && r.indexOf('###') > -1 ? r.replace(Ve, '.') : r),
	de = r => !r || g(r),
	z = (r, e, t) => {
		...
		return de(r) ? {} : { obj: r, k: he(i[s]) };
	},
	pe = (r, e, t) => {
		const { obj: i, k: s } = z(r, e, Object);
		...
		for (; o.obj === void 0 && a.length; )
			((n = `${a[a.length - 1]}.${n}`),
				...
				(o = z(r, a, Object)),
				o?.obj && typeof o.obj[`${o.k}.${n}`] < 'u' && (o.obj = void 0));
		o.obj[`${o.k}.${n}`] = t;
	},
	Ke = (r, e, t, i) => {
		const { obj: s, k: n } = z(r, e, Object);
		((s[n] = s[n] || []), s[n].push(t));
	},
	Q = (r, e) => {
		const { obj: t, k: i } = z(r, e);
	...
	$e = (r, e, t) => {
		for (const i in e)
			i !== '__proto__' &&
				i !== 'constructor' &&
				(i in r
					? g(r[i]) || r[i] instanceof String || g(e[i]) || e[i] instanceof String
						? t && (r[i] = e[i])
						: $e(r[i], e[i], t)
					: (r[i] = e[i]));
		return r;
	...
	A = r => r.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, '\\$&');
var Ue = { '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;', '/': '&#x2F;' };
const He = r => (g(r) ? r.replace(/[&<>"'\/]/g, e => Ue[e]) : r);
class Me {
	constructor(e) {
...
const ze = [' ', ',', '?', '!', ';'],
	...
	_e = (r, e, t) => {
		((e = e || ''), (t = t || ''));
		const i = ze.filter(a => e.indexOf(a) < 0 && t.indexOf(a) < 0);
		if (i.length === 0) return !0;
		const s = Be.getRegExp(`(${i.map(a => (a === '?' ? '\\?' : a)).join('|')})`);
		let n = !s.test(r);
		if (!n) {
			const a = r.indexOf(t);
...
class q {
	...
	create(e) {
		return new q(this.logger, { prefix: `${this.prefix}:${e}:`, ...this.options });
	}
	clone(e) {
	...
	emit(e, ...t) {
		(this.observers[e] &&
			Array.from(this.observers[e].entries()).forEach(([s, n]) => {
				for (let a = 0; a < n; a++) s(...t);
			}),
			this.observers['*'] &&
				Array.from(this.observers['*'].entries()).forEach(([s, n]) => {
					for (let a = 0; a < n; a++) s.apply(s, [e, ...t]);
				}));
	}
}
class ge extends X {
	...
	getResource(e, t, i, s = {}) {
		...
		e.indexOf('.') > -1
			? (o = e.split('.'))
			: ((o = [e, t]), i && (Array.isArray(i) ? o.push(...i) : g(i) && n ? o.push(...i.split(n)) : o.push(i)));
		const l = Q(this.data, o);
		return (
			!l && !t && !i && e.indexOf('.') > -1 && ((e = o[0]), (t = o[1]), (i = o.slice(2).join('.'))),
			l || !a || !g(i) ? l : re(this.data?.[e]?.[t], i, n)
		);
	}
	...
	addResources(e, t, i, s = { silent: !1 }) {
		for (const n in i) (g(i[n]) || Array.isArray(i[n])) && this.addResource(e, t, n, i[n], { silent: !0 });
		s.silent || this.emit('added', e, t, i);
	}
	addResourceBundle(e, t, i, s, n, a = { silent: !1, skipCopy: !1 }) {
		let o = [e, t];
...
function We() {
	...
	let t;
	return (
		(e.get = (i, s) => (t?.revoke?.(), s === ke ? r : (r.push(s), (t = Proxy.revocable(i, e)), t.proxy))),
...
const me = {},
	ee = r => !g(r) && typeof r != 'boolean' && typeof r != 'number';
class Z extends X {
	constructor(e, t = {}) {
		(super(),
			De(
	...
	extractFromKey(e, t) {
		...
		if (a && !o) {
			...
			if (l && l.length > 0) return { key: e, namespaces: g(n) ? [n] : n };
			const u = e.split(i);
			((i !== s || (i === s && this.options.ns.indexOf(u[0]) > -1)) && (n = u.shift()), (e = u.join(s)));
		}
		return { key: e, namespaces: g(n) ? [n] : n };
	}
	translate(e, t, i) {
		...
		if (
			...
			return '';
		(typeof e == 'function' && (e = ae(e, { ...this.options, ...s })), Array.isArray(e) || (e = [String(e)]));
		...
		if (f?.toLowerCase() === 'cimode')
			return h
				? n
					...
					: `${u}${c}${o}`
				: n
					? { res: o, usedKey: o, exactUsedKey: o, usedLng: f, usedNS: u, usedParams: this.getUsedParamsDetails(s) }
					: o;
		...
		const y = p?.usedKey || o,
			...
			O = s.joinArrays !== void 0 ? s.joinArrays : this.options.joinArrays,
			C = !this.i18nFormat || this.i18nFormat.handleAsObject,
			w = s.count !== void 0 && !g(s.count),
			R = Z.hasDefaultValue(s),
			j = w ? this.pluralResolver.getSuffix(f, s.count, s) : '',
			T = s.ordinal && w ? this.pluralResolver.getSuffix(f, s.count, { ordinal: !1 }) : '',
			J = w && !s.ordinal && s.count === 0,
			x =
				(J && s[`defaultValue${this.options.pluralSeparator}zero`]) ||
				s[`defaultValue${j}`] ||
				s[`defaultValue${T}`] ||
				s.defaultValue;
		let m = d;
		C && !d && R && (m = x);
		const S = ee(m),
			L = Object.prototype.toString.apply(m);
		if (C && m && S && $.indexOf(L) < 0 && !(g(O) && Array.isArray(m))) {
			if (!s.returnObjects && !this.options.returnObjects) {
				this.options.returnedObjectHandler ||
					this.logger.warn('accessing an object - but returnObjects options is not enabled!');
				const v = this.options.returnedObjectHandler
					? this.options.returnedObjectHandler(y, m, { ...s, ns: l })
					: `key '${o} (${this.language})' returned an object instead of string.`;
				return n ? ((p.res = v), (p.usedParams = this.getUsedParamsDetails(s)), p) : v;
			}
			if (a) {
				...
				for (const F in m)
					if (Object.prototype.hasOwnProperty.call(m, F)) {
						const D = `${W}${a}${F}`;
						(R && !d
							? (N[F] = this.translate(D, { ...s, defaultValue: ee(x) ? x[F] : void 0, joinArrays: !1, ns: l }))
							: (N[F] = this.translate(D, { ...s, joinArrays: !1, ns: l })),
							N[F] === D && (N[F] = m[F]));
					}
				d = N;
			}
		} else if (C && g(O) && Array.isArray(d)) ((d = d.join(O)), d && (d = this.extendTranslation(d, e, s, i)));
		else {
			let v = !1,
				N = !1;
			(!this.isValidLookup(d) && R && ((v = !0), (d = x)), this.isValidLookup(d) || ((N = !0), (d = o)));
			const F = (s.missingKeyNoValueFallbackToKey || this.options.missingKeyNoValueFallbackToKey) && N ? void 0 : d,
				D = R && x !== d && this.options.updateMissing;
			if (N || v || D) {
				if ((this.logger.log(D ? 'updateKey' : 'missingKey', f, u, o, D ? x : d), a)) {
					const E = this.resolve(o, { ...s, keySeparator: !1 });
					E &&
						E.res &&
						this.logger.warn(
				...
				if (this.options.saveMissingTo === 'fallback' && Y && Y[0]) for (let E = 0; E < Y.length; E++) U.push(Y[E]);
				else
					this.options.saveMissingTo === 'all'
						? (U = this.languageUtils.toResolveHierarchy(s.lng || this.language))
						: U.push(s.lng || this.language);
				const ue = (E, V, H) => {
					const fe = R && H !== d ? H : F;
					(this.options.missingKeyHandler
						? this.options.missingKeyHandler(E, u, V, fe, D, s)
						: this.backendConnector?.saveMissing && this.backendConnector.saveMissing(E, u, V, fe, D, s),
						this.emit('missingKey', E, u, V, d));
				};
				this.options.saveMissing &&
					(this.options.saveMissingPlurals && w
						? U.forEach(E => {
								const V = this.pluralResolver.getSuffixes(E, s);
								(J &&
									...
									V.forEach(H => {
										ue([E], o + H, s[`defaultValue${H}`] || x);
									}));
							})
						: ue(U, o, x));
			}
			((d = this.extendTranslation(d, e, s, p, i)),
				N && d === o && this.options.appendNamespaceToMissingKey && (d = `${u}${c}${o}`),
				(N || v) &&
		...
		return n ? ((p.res = d), (p.usedParams = this.getUsedParamsDetails(s)), p) : d;
	}
	extendTranslation(e, t, i, s, n) {
		if (this.i18nFormat?.parse)
			...
			i.interpolation &&
				this.interpolator.init({ ...i, interpolation: { ...this.options.interpolation, ...i.interpolation } });
			const l =
				g(e) &&
			...
			let c = i.replace && !g(i.replace) ? i.replace : i;
			if (
				(this.options.interpolation.defaultVariables && (c = { ...this.options.interpolation.defaultVariables, ...c }),
				(e = this.interpolator.interpolate(e, c, i.lng || this.language || s.usedLng, i)),
				l)
			) {
				const f = e.match(this.interpolator.nestingRegexp),
					h = f && f.length;
			...
			(!i.lng && s && s.res && (i.lng = this.language || s.usedLng),
				i.nest !== !1 &&
					(e = this.interpolator.nest(
						...
						(...f) =>
							n?.[0] === f[0] && !i.context
								? (this.logger.warn(`It seems you are nesting recursively key: ${f[0]} in key: ${t[0]}`), null)
								: this.translate(...f, t),
						i,
					)),
	...
	resolve(e, t = {}) {
		...
		return (
			...
			e.forEach(l => {
				...
				const h = t.count !== void 0 && !g(t.count),
					p = h && !t.ordinal && t.count === 0,
					d = t.context !== void 0 && (g(t.context) || typeof t.context == 'number') && t.context !== '',
				...
				f.forEach(b => {
					this.isValidLookup(i) ||
						...
						y.forEach($ => {
							...
							a = $;
							const O = [c];
							if (this.i18nFormat?.addLookupKeys) this.i18nFormat.addLookupKeys(O, c, $, b, t);
							else {
								let w;
								h && (w = this.pluralResolver.getSuffix($, t.count, t));
								const R = `${this.options.pluralSeparator}zero`,
									j = `${this.options.pluralSeparator}ordinal${this.options.pluralSeparator}`;
								if (
									(h &&
										(t.ordinal && w.indexOf(j) === 0 && O.push(c + w.replace(j, this.options.pluralSeparator)),
										O.push(c + w),
										p && O.push(c + R)),
									d)
								) {
									...
									(O.push(T),
										h &&
											(t.ordinal && w.indexOf(j) === 0 && O.push(T + w.replace(j, this.options.pluralSeparator)),
											O.push(T + w),
											p && O.push(T + R)));
								}
							}
							let C;
							for (; (C = O.pop()); ) this.isValidLookup(i) || ((n = C), (i = this.getResource($, b, C, t)));
						}));
				});
			}),
			{ res: i, usedKey: s, exactUsedKey: n, usedLng: a, usedNS: o }
		);
	}
	isValidLookup(e) {
	...
	toResolveHierarchy(e, t) {
		...
		return (
			g(e) && (e.indexOf('-') > -1 || e.indexOf('_') > -1)
				...
				: g(e) && n(this.formatLanguageCode(e)),
			i.forEach(a => {
...
const ye = { zero: 0, one: 1, two: 2, few: 3, many: 4, other: 5 },
	Se = { select: r => (r === 1 ? 'one' : 'other'), resolvedOptions: () => ({ pluralCategories: ['one', 'other'] }) };
class Ye {
	constructor(e, t = {}) {
	...
	getSuffixes(e, t = {}) {
		...
		return (
			...
			i
				? i
						.resolvedOptions()
						.pluralCategories.sort((s, n) => ye[s] - ye[n])
...
const be = (r, e, t, i = '.', s = !0) => {
		let n = Ae(r, e, t);
		return (!n && s && g(t) && ((n = re(r, t, i)), n === void 0 && (n = re(e, t, i))), n);
	},
	te = r => r.replace(/\$/g, '$$$$');
class Oe {
	constructor(e = {}) {
	...
	init(e = {}) {
		...
		((this.escape = t !== void 0 ? t : He),
			...
			(this.nestingPrefix = h ? A(h) : p || A('$t(')),
			(this.nestingSuffix = d ? A(d) : y || A(')')),
	...
	interpolate(e, t, i, s) {
		...
		const l = (this.options && this.options.interpolation && this.options.interpolation.defaultVariables) || {},
			u = p => {
				if (p.indexOf(this.formatSeparator) < 0) {
					const $ = be(t, l, p, this.options.keySeparator, this.options.ignoreJSONStructure);
					return this.alwaysFormat ? this.format($, void 0, i, { ...s, ...t, interpolationkey: p }) : $;
				}
				const d = p.split(this.formatSeparator),
					y = d.shift().trim(),
					b = d.join(this.formatSeparator).trim();
				return this.format(be(t, l, y, this.options.keySeparator, this.options.ignoreJSONStructure), b, i, {
					...s,
					...t,
					interpolationkey: y,
				});
		...
		this.resetRegExp();
		const c = s?.missingInterpolationHandler || this.options.missingInterpolationHandler,
			f =
		...
		return (
			[
				{ regex: this.regexpUnescape, safeValue: p => te(p) },
				{ regex: this.regexp, safeValue: p => (this.escapeValue ? te(this.escape(p)) : te(p)) },
			].forEach(p => {
				for (o = 0; (n = p.regex.exec(e)); ) {
					...
					if (((a = u(d)), a === void 0))
						if (typeof c == 'function') {
							const b = c(e, n, s);
							a = g(b) ? b : '';
						} else if (s && Object.prototype.hasOwnProperty.call(s, d)) a = '';
						else if (f) {
							a = n[0];
							continue;
						} else (this.logger.warn(`missed to pass in variable ${d} for interpolating ${e}`), (a = ''));
					else !g(a) && !this.useRawValueToEscape && (a = ce(a));
					const y = p.safeValue(a);
					if (
						((e = e.replace(n[0], y)),
						f ? ((p.regex.lastIndex += a.length), (p.regex.lastIndex -= n[0].length)) : (p.regex.lastIndex = 0),
						o++,
						o >= this.maxReplaces)
					)
						break;
	...
	nest(e, t, i = {}) {
		...
		const o = (l, u) => {
			...
			const p = h.match(/'/g),
				d = h.match(/"/g);
			(((p?.length ?? 0) % 2 === 0 && !d) || d.length % 2 !== 0) && (h = h.replace(/'/g, '"'));
			try {
				((a = JSON.parse(h)), u && (a = { ...u, ...a }));
			} catch (y) {
				return (this.logger.warn(`failed parsing options string in nesting for key ${l}`, y), `${l}${c}${h}`);
			}
		...
		for (; (s = this.nestingRegexp.exec(e)); ) {
			let l = [];
			((a = { ...i }),
				(a = a.replace && !g(a.replace) ? a.replace : a),
			...
			if (
				(u !== -1 &&
					((l = s[1]
						...
						.filter(Boolean)),
					(s[1] = s[1].slice(0, u))),
				(n = t(o.call(this, s[1].trim(), a), a)),
				n && s[0] === e && !g(n))
			)
				return n;
			(g(n) || (n = ce(n)),
				n || (this.logger.warn(`missed to resolve ${s[1]} for nesting ${e}`), (n = '')),
				l.length &&
					(n = l.reduce((c, f) => this.format(c, f, i.lng, { ...i, interpolationkey: s[1].trim() }), n.trim())),
				(e = e.replace(s[0], n)),
...
const Ge = r => {
	...
	Le = r => {
		...
		return (t, i, s) => {
			...
			let o = e[a];
			return (o || ((o = r(_(i), s)), (e[a] = o)), o(t));
		};
	},
	Qe = r => (e, t, i) => r(_(t), i)(e);
class qe {
	constructor(e = {}) {
	...
	init(e, t = { interpolation: {} }) {
		...
		const i = t.cacheInBuiltFormats ? Le : Qe;
		this.formats = {
			number: i((s, n) => {
				const a = new Intl.NumberFormat(s, { ...n });
				return o => a.format(o);
			}),
			currency: i((s, n) => {
				const a = new Intl.NumberFormat(s, { ...n, style: 'currency' });
				return o => a.format(o);
			}),
			datetime: i((s, n) => {
				const a = new Intl.DateTimeFormat(s, { ...n });
				return o => a.format(o);
			}),
			relativetime: i((s, n) => {
				const a = new Intl.RelativeTimeFormat(s, { ...n });
				return o => a.format(o, n.range || 'day');
			}),
			list: i((s, n) => {
				const a = new Intl.ListFormat(s, { ...n });
				return o => a.format(o);
			}),
		};
	}
	...
	format(e, t, i, s = {}) {
		...
		return n.reduce((o, l) => {
			...
			if (this.formats[u]) {
				let f = o;
				try {
					const h = s?.formatParams?.[s.interpolationkey] || {},
						p = h.locale || h.lng || s.locale || s.lng || i;
					f = this.formats[u](o, p, { ...c, ...s, ...h });
				} catch (h) {
					this.logger.warn(h);
				}
				return f;
...
class Xe extends X {
	...
	loaded(e, t, i) {
		...
		(t && this.emit('failedLoading', n, a, t),
			!t && i && this.store.addResourceBundle(n, a, i, void 0, void 0, { skipCopy: !0 }),
			(this.state[e] = t ? -1 : 2),
			t && i && (this.state[e] = 0));
		const o = {};
	...
	read(e, t, i, s = 0, n = this.retryTimeout, a) {
		...
		if (this.readingCalls >= this.maxParallelReads) {
			this.waitingReads.push({ lng: e, ns: t, fcName: i, tried: s, wait: n, callback: a });
			return;
		}
		this.readingCalls++;
		const o = (u, c) => {
	...
	loadOne(e, t = '') {
		...
		this.read(s, n, 'read', void 0, void 0, (a, o) => {
			(a && this.logger.warn(`${t}loading namespace ${n} for language ${s} failed`, a),
				!a && o && this.logger.log(`${t}loaded namespace ${n} for language ${s}`, o),
				this.loaded(e, a, o));
		});
	}
	saveMissing(e, t, i, s, n, a = {}, o = () => {}) {
		...
		if (!(i == null || i === '')) {
			if (this.backend?.create) {
				const l = { ...a, isUpdate: n },
					u = this.backend.create.bind(this.backend);
				if (u.length < 6)
					try {
						let c;
						(u.length === 5 ? (c = u(e, t, i, s, l)) : (c = u(e, t, i, s)),
...
const se = () => ({
		...
		overloadTranslationOptionHandler: r => {
			...
			if (
				...
				g(r[1]) && (e.defaultValue = r[1]),
				g(r[2]) && (e.tDescription = r[2]),
	...
	we = r => (
		g(r.ns) && (r.ns = [r.ns]),
		g(r.fallbackLng) && (r.fallbackLng = [r.fallbackLng]),
		g(r.fallbackNS) && (r.fallbackNS = [r.fallbackNS]),
		r.supportedLngs?.indexOf?.('cimode') < 0 && (r.supportedLngs = r.supportedLngs.concat(['cimode'])),
...
const tt = r =>
	!!(
		r?.modules?.backend?.name?.indexOf('Locize') > 0 ||
		r?.modules?.backend?.constructor?.name?.indexOf('Locize') > 0 ||
		(r?.options?.backend?.backends &&
			r.options.backend.backends.some(
...
class B extends X {
	...
	init(e = {}, t) {
		((this.isInitializing = !0),
			...
			e.defaultNS == null &&
				e.ns &&
				(g(e.ns) ? (e.defaultNS = e.ns) : e.ns.indexOf('translation') < 0 && (e.defaultNS = e.ns[0])));
		const i = se();
		((this.options = { ...i, ...this.options, ...we(e) }),
		...
		if (!this.options.isClone) {
			this.modules.logger ? I.init(s(this.modules.logger), this.options) : I.init(null, this.options);
			let u;
			this.modules.formatter ? (u = this.modules.formatter) : (u = qe);
			...
			((f.logger = I),
				...
				(f.interpolator = new Oe(this.options)),
				(f.utils = { hasLoadedNamespace: this.hasLoadedNamespace.bind(this) }),
				(f.backendConnector = new Xe(s(this.modules.backend), f.resourceStore, f, this.options)),
				f.backendConnector.on('*', (p, ...d) => {
					this.emit(p, ...d);
				}),
				...
				this.modules.i18nFormat &&
					((f.i18nFormat = s(this.modules.i18nFormat)), f.i18nFormat.init && f.i18nFormat.init(this)),
				(this.translator = new Z(this.services, this.options)),
				this.translator.on('*', (p, ...d) => {
					this.emit(p, ...d);
				}),
	...
	loadResources(e, t = G) {
		...
		if ((typeof e == 'function' && (i = e), !this.options.resources || this.options.partialBundledLanguages)) {
			...
			(s ? a(s) : this.services.languageUtils.getFallbackCodes(this.options.fallbackLng).forEach(l => a(l)),
				...
				this.services.backendConnector.load(n, this.options.ns, o => {
			...
			a = o => {
				...
				const l = g(o) ? o : o && o[0],
					u = this.store.hasLanguageSomeTranslations(l)
						? l
						: this.services.languageUtils.getBestMatchFromCodes(g(o) ? [o] : o);
				(u &&
					(this.language || s(u),
	...
	getFixedT(e, t, i) {
		const s = (n, a, ...o) => {
			let l;
			(typeof a != 'object' ? (l = this.options.overloadTranslationOptionHandler([n, a].concat(o))) : (l = { ...a }),
				(l.lng = l.lng || s.lng),
			...
			return (
				l.keyPrefix && Array.isArray(n)
					? (c = n.map(
							f => (typeof f == 'function' && (f = ae(f, { ...this.options, ...a })), `${l.keyPrefix}${u}${f}`),
						))
					: (typeof n == 'function' && (n = ae(n, { ...this.options, ...a })),
						(c = l.keyPrefix ? `${l.keyPrefix}${u}${n}` : n)),
				this.t(c, l)
			);
		};
	...
	loadLanguages(e, t) {
		...
		const s = this.options.preload || [],
			n = e.filter(a => s.indexOf(a) < 0 && this.services.languageUtils.isSupportedCode(a));
		return n.length
	...
	dir(e) {
		...
		const t = [
			...
			i = this.services?.languageUtils || new xe(se());
	...
	cloneInstance(e = {}, t = G) {
		...
		if (e.interpolation) {
			const l = { ...se().interpolation, ...this.options.interpolation, ...e.interpolation },
				u = { ...s, interpolation: l };
			n.services.interpolator = new Oe(u);
		}
		return (
			(n.translator = new Z(n.services, s)),
			n.translator.on('*', (o, ...l) => {
				n.emit(o, ...l);
			}),
			n.init(s, t),
...
const st = (r, e, t, i) => {
		const s = [t, { code: e, ...(i || {}) }];
		if (r?.services?.logger?.forward) return r.services.logger.forward(s, 'warn', 'react-i18next::', !0);
		(K(s[0]) && (s[0] = `react-i18next:: ${s[0]}`),
			r?.services?.logger?.warn ? r.services.logger.warn(...s) : console?.warn && console.warn(...s));
	},
	Ce = {},
	je = (r, e, t, i) => {
		(K(t) && Ce[t]) || (K(t) && (Ce[t] = new Date()), st(r, e, t, i));
	},
	Fe = (r, e) => () => {
	...
	oe = (r, e, t) => {
		r.loadNamespaces(e, Fe(r, t));
	},
	Re = (r, e, t, i) => {
		if ((K(t) && (t = [t]), r.options.preload && r.options.preload.indexOf(e) > -1)) return oe(r, t, i);
		(t.forEach(s => {
			r.options.ns.indexOf(s) < 0 && r.options.ns.push(s);
		}),
			r.loadLanguages(e, Fe(r, i)));
	},
	it = (r, e, t = {}) =>
		!e.languages || !e.languages.length
			? (je(e, 'NO_LANGUAGES', 'i18n.languages were undefined or empty', { languages: e.languages }), !0)
			: e.hasLoadedNamespace(r, {
					...
					precheck: (i, s) => {
						if (
							...
							!s(i.isLanguageChangingTo, r)
						)
							return !1;
					},
				}),
	K = r => typeof r == 'string',
	nt = r => typeof r == 'object' && r !== null,
...
function gt() {
	...
	function o(f, h) {
		var p = h(),
			d = i({ inst: { value: p, getSnapshot: h } }),
			y = d[0].inst,
			b = d[1];
		return (
			n(
				function () {
					((y.value = p), (y.getSnapshot = h), l(y) && b({ inst: y }));
				},
				[f, p, h],
			),
			s(
	...
	return ((ne.useSyncExternalStore = r.useSyncExternalStore !== void 0 ? r.useSyncExternalStore : c), ne);
}
var Pe;
function mt() {
	return (Pe || ((Pe = 1), (ie.exports = gt())), ie.exports);
}
var xt = mt();
const yt = (r, e) => (K(e) ? e : nt(e) && K(e.defaultValue) ? e.defaultValue : Array.isArray(r) ? r[r.length - 1] : r),
	St = { t: yt, ready: !1 },
	bt = () => () => {},
	wt = (r, e = {}) => {
		const { i18n: t } = e,
			{ i18n: i, defaultNS: s } = k.useContext(dt) || {},
			n = t || i || ht();
		(n && !n.reportNamespaces && (n.reportNamespaces = new pt()),
			n ||
				je(
					n,
					'NO_I18NEXT_INSTANCE',
		...
		const a = k.useMemo(() => ({ ...ft(), ...n?.options?.react, ...e }), [n, e]),
			{ useSuspense: o, keyPrefix: l } = a,
			u = s || n?.options?.defaultNS,
			c = K(u) ? [u] : u || ['translation'],
			f = k.useMemo(() => c, c);
		n?.reportNamespaces?.addUsedNamespaces?.(f);
		const h = k.useRef(0),
			p = k.useCallback(
				x => {
					if (!n) return bt;
					const { bindI18n: m, bindI18nStore: S } = a,
						L = () => {
							((h.current += 1), x());
						};
					return (
						...
						S && n.store.on(S, L),
						() => {
							(m && m.split(' ').forEach(v => n.off(v, L)), S && S.split(' ').forEach(v => n.store.off(v, L)));
						}
					);
				},
				[n, a],
			...
			y = k.useCallback(() => {
				if (!n) return St;
				const x = !!(n.isInitialized || n.initializedStoreOnce) && f.every(W => it(W, n, a)),
					m = e.lng || n.language,
					S = h.current,
					L = d.current;
				if (L && L.ready === x && L.lng === m && L.keyPrefix === l && L.revision === S) return L;
				const N = {
					t: n.getFixedT(m, a.nsMode === 'fallback' ? f : f[0], l),
					ready: x,
					lng: m,
					keyPrefix: l,
					revision: S,
			...
			[b, $] = k.useState(0),
			{ t: O, ready: C } = xt.useSyncExternalStore(p, y, y);
		k.useEffect(() => {
			if (n && !C && !o) {
				const x = () => $(m => m + 1);
				e.lng ? Re(n, e.lng, f, x) : oe(n, f, x);
			}
		}, [n, e.lng, f, C, o, b]);
		const w = n || {},
			R = k.useRef(null),
			...
			T = x => {
				...
				const S = Object.create(Object.getPrototypeOf(x), m);
				if (!Object.prototype.hasOwnProperty.call(S, '__original'))
					try {
						Object.defineProperty(S, '__original', { value: x, writable: !1, enumerable: !1, configurable: !1 });
					} catch {}
				return S;
			},
			J = k.useMemo(() => {
				...
				let S = x;
				x &&
					(R.current && R.current.__original === x
						? j.current !== m
							? ((S = T(x)), (R.current = S), (j.current = m))
							: (S = R.current)
						: ((S = T(x)), (R.current = S), (j.current = m)));
				const L = [O, S, C];
				return ((L.t = O), (L.i18n = S), (L.ready = C), L);
			}, [O, w, C, w.resolvedLanguage, w.language, w.languages]);
		if (n && o && !C)
			throw new Promise(x => {
				const m = () => x();
				e.lng ? Re(n, e.lng, f, m) : oe(n, f, m);
			});
		return J;
	};
export { Lt as a, P as i, wt as u };
