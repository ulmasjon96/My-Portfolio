import{r as y,j as t}from"./index-CWivW2Br.js";import{S as M}from"./section-title-BPwu7c1U.js";import{L as S,d as k,m as b}from"./features-animation-vtX01uMY.js";function E(){const w=y.useRef(null),h=y.useRef([]),u=y.useRef();return y.useEffect(()=>{const r=w.current;if(!r)return;const s=r.getContext("2d");if(!s)return;let e=0,o=0;const c=.5,a=150,x=60,d=()=>{const i=r.parentElement;if(i){e=r.width=i.offsetWidth,o=r.height=i.offsetHeight,h.current=[];for(let n=0;n<x;n++)h.current.push({x:Math.random()*e,y:Math.random()*o,vx:(Math.random()-.5)*c,vy:(Math.random()-.5)*c})}};d(),window.addEventListener("resize",d);const p=(i,n)=>{const l=i.x-n.x,m=i.y-n.y;return Math.sqrt(l*l+m*m)},f=()=>{const i=h.current;for(let n=0;n<i.length-1;n++)for(let l=n+1;l<i.length;l++){const m=p(i[n],i[l]);if(m<=a){const N=1-m/a;s.strokeStyle=`hsla(177, 100%, 37%, ${N})`,s.lineWidth=Math.pow(1-m/a,1.5)*2,s.beginPath(),s.moveTo(i[n].x,i[n].y),s.lineTo(i[l].x,i[l].y),s.stroke()}}},g=()=>{const i=h.current;for(const n of i)s.beginPath(),s.arc(n.x,n.y,2,0,Math.PI*2),s.fillStyle="hsl(177, 100%, 47%)",s.fill()},v=()=>{const i=h.current;for(const n of i)n.x+=n.vx,n.y+=n.vy,(n.x<0||n.x>e)&&(n.vx*=-1),(n.y<0||n.y>o)&&(n.vy*=-1)},j=()=>{s.clearRect(0,0,e,o),f(),g(),v(),u.current=requestAnimationFrame(j)};return j(),()=>{window.removeEventListener("resize",d),u.current&&cancelAnimationFrame(u.current)}},[]),t.jsx("canvas",{ref:w,className:"absolute inset-0 w-full h-full"})}const D=`// Get the canvas element and set up the 2D context
